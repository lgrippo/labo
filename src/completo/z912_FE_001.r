...
#   dataset[ , mvr_mpagominimo         := mv_mpagominimo  / mv_mlimitecompra ]
  ...
  dataset[ , ctotal_ctrx             := rowSums( cbind( ctarjeta_debito_trx, ctarjeta_visa_trx, ctarjeta_master_trx) , na.rm=TRUE ) ]  
  dataset[ , ctotal_payroll_trx       := rowSums( cbind( cpayroll_trx, cpayroll2_trx) , na.rm=TRUE ) ]  
  dataset[ , ctotal_com_y_marg        := rowSums( cbind( cpayroll_trx, cpayroll2_trx) , na.rm=TRUE ) ]  
  
  #Ratios
  
  #dataset[ , rmrentabilidad         := mrentabilidad  / mrentabilidad_annual ]
  dataset[ , rmpasivo_margen_total   := mpasivos_margen   / ctotal_com_y_marg ]
  ...
  dataset[ , rccaja_ahorro_tcuentas  := ccaja_ahorro   / tcuentas ]
  dataset[ , rccaja_ahorro_cproductos:= ccaja_ahorro   / cproductos ]
  dataset[ , rmpayroll_cpayroll_trx  := mpayroll   / cpayroll_trx ]
  dataset[ , rcpayroll_trx_total     := cpayroll_trx   / ctotal_payroll_trx ]
  dataset[ , rctarjeta_visa_trx_total:= ctarjeta_visa_trx   / ctotal_ctrx ]
  dataset[ , rctotal_trx_tpayroll_trx:= ctotal_ctrx / ctotal_payroll_trx]
  dataset[ , rmtarjeta_visa_consumo_total:= mtarjeta_visa_consumo / mtotal_consumos]
