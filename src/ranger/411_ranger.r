...
param  <- list( "num.trees"=      2030,  #cantidad de arboles
                "mtry"=             9,  #cantidad de variables que evalua para hacer un split  sqrt(ncol(dtrain))
                "min.node.size"=  3,  #tamaño minimo de las hojas
                "max.depth"=    8   # 0 significa profundidad infinita
              )

set.seed(596293) #Establezco la semilla aleatoria
...
archivo_salida  <- "./exp/KA2411/KA_411_002.csv"
